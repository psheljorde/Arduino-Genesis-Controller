...
## Software Setup
  ...
  3.The Arduino UNO R2/R3 uses a MEGA16u2 chip to upload sketches to memory. 
  * In order to be recognized as an HID USB device,
  we must flash the 16u2 chip with some custom firmware. 
  * To do this on the Arduino R3, short the 2 pins closest to the USB port to enter
  DFU mode. From there, run UnoJoy/TurnIntoAJoystick.bat. 
  * Now that you have reflashed the Arduino, unplug it from the USB and re-plug it
